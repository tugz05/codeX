...
import { ArrowLeft, Edit, FileText, Clock, Award, Users, BarChart3, CheckCircle2, Shield } from 'lucide-vue-next'
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
      ...
      <div class="flex items-center justify-between">
        ...
        <Link :href="route('instructor.examinations.edit', [props.classlist.id, props.examination.id])" as="button">
          <Button>
            <Edit class="mr-2 h-4 w-4" /> Edit Examination
          </Button>
        </Link>
      </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <CardContent class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span class="ml-2 font-medium">{{ props.examination.total_points }}</span>
                </div>
                <div>
                  <span class="text-muted-foreground">Time Limit:</span>
                  <span class="ml-2 font-medium">{{ props.examination.time_limit ? `${props.examination.time_limit} minutes` : 'No limit' }}</span>
                </div>
                <div>
                  <span class="text-muted-foreground">Attempts Allowed:</span>
                  <span class="ml-2 font-medium">{{ props.examination.attempts_allowed }}</span>
                </div>
                <div>
                  <span class="text-muted-foreground">Status:</span>
                  <Badge :variant="props.examination.is_published ? 'default' : 'secondary'" class="ml-2">
                    {{ props.examination.is_published ? 'Published' : 'Draft' }}
                  </Badge>
                </div>
                <div>
                  <span class="text-muted-foreground">Start Date:</span>
                  <span class="ml-2 font-medium">{{ formatDate(props.examination.start_date) }}</span>
                </div>
                <div>
                  <span class="text-muted-foreground">End Date:</span>
                  <span class="ml-2 font-medium">{{ formatDate(props.examination.end_date) }}</span>
                </div>
              </div>

              <div class="flex flex-wrap gap-2">
                <Badge v-if="props.examination.shuffle_questions" variant="outline">
                  Shuffle Questions
                </Badge>
                <Badge v-if="props.examination.show_correct_answers" variant="outline">
                  Show Correct Answers
                </Badge>
                <Badge v-if="props.examination.require_proctoring" variant="outline" class="flex items-center gap-1">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{{ props.examination.tests.length }} test(s) • {{ props.examination.total_points }} total points</CardDescription>
            </CardHeader>
            <CardContent class="space-y-6">
              <div v-for="(test, tIndex) in props.examination.tests" :key="test.id" class="rounded-lg border p-5 space-y-4">
          ...
          <Card>
            ...
            <CardContent class="space-y-4">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="font-semibold">{{ props.statistics.average_score }}/{{ props.examination.total_points }}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium">{{ props.examination.tests.length }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-muted-foreground">Total Questions:</span>
                <span class="font-medium">
                  {{ props.examination.tests.reduce((sum, test) => sum + test.questions.length, 0) }}
                </span>
              </div>
              <div class="flex justify-between">
                <span class="text-muted-foreground">Created:</span>
                <span class="font-medium">{{ formatDate(props.examination.created_at) }}</span>
              </div>
            </CardContent>
          </Card>
        </div>

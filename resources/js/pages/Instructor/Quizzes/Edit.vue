...
const form = useForm({
  ...
  attempts_allowed: props.quiz.attempts_allowed,
...
onMounted(() => {
  // Load existing tests
  tests.value = props.quiz.tests.map(test => ({
    title: test.title,
    type: test.type || '',
...
function submit() {
  ...
  form.put(route('instructor.quizzes.update', [props.classlist.id, props.quiz.id]), {
    onSuccess: () => {
      toast.success('Quiz updated successfully.')
      router.visit(route('instructor.activities.index', props.classlist.id))
    },
    onError: () => toast.error('Unable to update quiz.'),
          ...
          <Card>
            ...
            <CardContent class="space-y-6">
              ...
              <Alert v-if="tests.length === 0" variant="default">
                <AlertDescription>Add at least one test to create the quiz.</AlertDescription>
              </Alert>
            </CardContent>
          </Card>
        </div>

...
const props = defineProps<{
  ...
  quiz: { id: number; title: string; time_limit: number | null; total_points: number }
...
const antiCheating = useAntiCheating(props.attempt.id, 'QuizAttempt')
...
onMounted(() => {
  ...
  if (props.quiz.time_limit) {
    ...
    const timeLimitMs = props.quiz.time_limit * 60 * 1000
...
function saveAnswer(questionId: number, answer: string | string[]) {
  ...
  router.post(
    route('student.quizzes.saveAnswer', [props.classlist.id, props.quiz.id, props.attempt.id]),
...
function submitQuiz() {
  ...
  router.post(
    route('student.quizzes.submit', [props.classlist.id, props.quiz.id, props.attempt.id]),
    {},
    {
      onSuccess: () => {
      ...
      onError: (errors) => {
        ...
        const errorMessage = errors?.error || errors?.message || 'Failed to submit quiz'
...
<template>
  <QuizExamLayout :title="`Taking Quiz: ${props.quiz.title}`">
    <div class="min-h-screen flex flex-col">
      ...
      <div class="flex items-center justify-between rounded-lg border bg-card p-4">
        <div>
          <h1 class="text-lg font-semibold">{{ props.quiz.title }}</h1>
      ...
      <Card v-if="props.questions.length === 0" class="flex-1">
        <CardContent class="flex flex-col items-center justify-center py-12">
          ...
          <p class="text-sm text-muted-foreground text-center">
            This quiz does not have any questions yet. Please contact your instructor.
          </p>
        </CardContent>
      </Card>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          >
            <Send class="mr-2 h-4 w-4" /> {{ isSubmitting ? 'Submitting...' : 'Submit Quiz' }}
          </Button>
        </div>
      </div>
      </div>

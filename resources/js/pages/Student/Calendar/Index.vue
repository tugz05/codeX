...
const exportCalendar = () => {
  ...
  window.location.href = route('student.calendar.export') + '?' + params.toString();
};
...
const handleEventClick = (info: any) => {
  ...
  const routes: Record<string, (classlistId: string, id: string) => string> = {
    activity: (cl, id) => route('student.activities.show', [cl, id]),
    assignment: (cl, id) => route('student.assignments.show', [cl, id]),
    quiz: (cl, id) => route('student.quizzes.show', [cl, id]),
    examination: (cl, id) => route('student.examinations.show', [cl, id]),
  };
            ...
            <div class="space-y-3">
              ...
              <div class="space-y-2 max-h-48 overflow-y-auto pr-1">
                <div 
                  ...
                  :key="classlist.id" 
                  class="flex items-center gap-2 p-1.5 rounded-md hover:bg-accent/50 transition-colors"
                >
                  <Checkbox
